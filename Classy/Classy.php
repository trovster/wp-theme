	...
	*/
	...
	public static function get_the_page() {
		...
		elseif(_site_is_section('events')) {
			$the_page = Classy_Page::find_by_id(_site_get_nav_item_id('education'));
		}
		elseif(_site_is_section('news-events')) {
			$the_page = Classy_Page::find_by_id(_site_get_nav_item_id('news-events'));
		}
		elseif(_site_is_section('team')) {
			$the_page = Classy_Page::find_by_id(_site_get_nav_item_id('team'));
		}
		elseif(_site_is_section('career')) {
			$the_page = Classy_Page::find_by_id(_site_get_nav_item_id('careers'));
		}
		elseif(_site_is_section('product')) {
			$the_page = Classy_Page::find_by_id(_site_get_nav_item_id('catalogue'));
		}
		elseif(_site_is_section('academy-board')) {
			$the_page = Classy_Page::find_by_id(_site_get_nav_item_id('academy-board'));
		}
	...
	public function get_the_navigation($include_top_level = true) {
		...
		if(isset($this->post) && $this->post->ID === _site_get_nav_item_id('news')) {
		...
		elseif(_site_is_section('product')) {
			$classy_product = is_singular('product') ? Classy_Product::find_by_id($post->ID) : new Classy_Product();
			return $classy_product->get_the_navigation($include_top_level);
		}
		elseif(_site_is_section('homepage')) {
			$pages			= array();
			$navigation		= _site_get_navigation();
			$i = 1; $total	= count($navigation);
			...
			foreach($navigation as $nav_key => $nav_item) {
				...
				$pages[]	= sprintf('<li class="%s"><a href="%s">%s</a></li>', implode(' ', $classes), $nav_item['href'], $nav_item['text']);
			}

			return count($pages) > 0 ? sprintf('<div class="section section-navigation nav"><ul>%s</ul></div>', implode("\r\n", $pages)) : '';
		}
		else {
			...
			$pages			= wp_list_pages(array(
				...
				'exclude'	=> $include_top_level ? $top_level_id : 0,
	...
	public function the_attr($type, $options = array()) {
		...
		switch($type) {
			...
			case 'id':
				$output = sprintf(' id="%s"', $this->get_post_type() . '-' . $this->get_the_ID());
				break;
