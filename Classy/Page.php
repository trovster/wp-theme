...
class Classy_Page extends Classy {
	...
	public function __construct($options = array()) {
		...
		if($options === 'initialize') {
			...
			add_filter('init',															array(&$this, 'init_register_rewrite_rules'));
		}
	 ...
	 */
	public function init_register_rewrite_rules() {
		add_rewrite_rule('^(([^/]+)/(page-[^/]+))(/[0-9]+)?/?$', 'index.php?pagename=$matches[1]&page=$matches[3]&section=$matches[2]', 'top');
	}
	...
	public function init_register_images() {
		add_image_size($this->get_post_type(), 800, 600, true);
	...
	public function the_content($more_link_text = NULL, $stripteaser = false) {
		if($this->has_first_column() && $this->has_second_column()) {
			$content_primary	= apply_filters('the_content', $this->get_the_content());
			$column_1			= apply_filters('the_content', $this->get_first_column());
			$column_2			= apply_filters('the_content', $this->get_second_column());
			$the_content		= sprintf('<div class="entry-content">%s</div><div class="columns"><div class="column column-1">%s</div><div class="column column-2">%s</div></div>', $content_primary, $column_1, $column_2);
		}
		else {
			$content_primary	= apply_filters('the_content', $this->get_the_content());
	...
	/**
	 * has_first_column
	 * @desc	Check whether a first column exists.
	 * @return	boolean
	 */
	public function has_first_column() {
		return $this->has_custom_value('first_column');
	}
	
	/**
	 * get_first_column
	 * @desc	Retrieve the first column.
	 * @return	string
	 */
	public function get_first_column() {
		return $this->has_first_column() ? $this->get_custom_value('first_column') : '';
	}
	
	/**
	 * the_first_column
	 * @desc	Output the first column.
	 * @output	string
	 */
	public function the_first_column() {
		echo $this->get_first_column();
	}
	...
	public function has_banner_image() {
		return class_exists('MultiPostThumbnails') && MultiPostThumbnails::has_post_thumbnail($this->get_post_type(), 'banner-landscape', $this->get_the_ID());
	}
	...
	public function get_banner_image($size = 'banner-landscape', $attr = '') {
		if($this->has_banner_image()) {
			return MultiPostThumbnails::get_the_post_thumbnail($this->get_post_type(), 'banner-landscape', $this->get_the_ID(), $size);
		}
	...
	public function has_listing_image() {
		return class_exists('MultiPostThumbnails') && MultiPostThumbnails::has_post_thumbnail($this->get_post_type(), 'listing', $this->get_the_ID());
	}
	...
	public function get_listing_image($size = 'page-listing', $attr = '') {
		if($this->has_listing_image()) {
			return MultiPostThumbnails::get_the_post_thumbnail($this->get_post_type(), 'listing', $this->get_the_ID(), $size);
		}
